...
const { getTimes, getPosition, getMoonPosition, getMoonIllumination } = suncalc
...
const getRainPercent = (code) => {
  const heavyCodes = [202, 212, 502, 503, 504, 522]
  if (heavyCodes.includes(code)) {
    return 100
...
const parseWeather = (key, data, settings) => {
  ...
  let weather = {
    ...
    condition_label: data.weather[0].description,
    id: data.id,
    hash_key: key,
